						...
						if ($this->auth->validate()) {
							...
							foreach ($this->requiredAttributes as $key => $attr) {
								if (isset($attributes[$attr[1]])) {
									...
									throw new EAuthException(Yii::t('EAuth.eauth', 'Unable to complete the authentication because the required data was not received.', array('{provider}' => ucfirst($this->getServiceName()))));
									return false;
								}
							}
