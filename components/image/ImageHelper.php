 ...
 * 'import'=>array(
 *     'ext.components.image.ImageHelper',
 * ),
 *
 * USAGE:
 * $thumb = Yii::app()->request->baseUrl.ImageHelper::thumb(100,100,'images/marilyn-monroe.jpg', array('resizeMethod'=>'resize'));
...
class ImageHelper
{
	...
	public static function thumb($width, $height, $img, $options=null)
	{
		...
		if(!file_exists($thumbPath.$thumbName) || filemtime($thumbPath.$thumbName) < filemtime($img)) {
			 Yii::import('ext.components.image.phpThumb.PhpThumbFactory');
