...
/**
 * When connecting to Linkedin I was gettin error:
 * oauth_problem="SCOPE_INVALID"
 *
 * To solve the problem I modified this class
 * by adding condition so that scope param is
 * attached to url only if scope is not empty
 *
 * if($scope) {
 *     $params['scope'] = $scope;
 * }
...
class EOAuthUtils extends EOAuthComponent
{
	 ...
	 */
	public static function GetRequestToken(OAuthConsumer $consumer,$scope,$endpoint,$applicationName,$callbackUrl)
	{
		...
		$request=OAuthRequest::from_consumer_and_token($consumer,NULL,'GET',$endpoint,$params);
	 ...
	 */
	public static function GetAccessToken(OAuthConsumer $consumer,OAuthToken $token,$verifier,$endpoint)
	{
		$signatureMethod=new OAuthSignatureMethod_HMAC_SHA1();
