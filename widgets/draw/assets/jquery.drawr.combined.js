...
(function( $ ) {
	...
	$.fn.drawr = function( action, param ) {
		...
		plugin.record_undo_entry = function(){
			this.$undoButton.css("color","#000000");
		...
		plugin.select_button = function(button){
			...
			$(button).css({"background": "#0088cc", "color" : "#ffffff"});
			...
			el.on("mousedown.drawr touchstart.drawr", function(e){
				...
				if($(this).data("type")=="toggle") {//toggle data attribute and select effect
					...
					if($(this).data("state")==true){
						$(this).css({"background" : "#0088cc", "color" : "#ffffff"});
					} else {
						$(this).css({"background" : "#eeeeee", "color" : "#000000"});
					}
				}
		...
		plugin.initialize_canvas = function(width,height,reset){
			...
			if(this.settings.enable_tranparency==true) {
				if(this.settings.hasOwnProperty("background_image") && this.settings.background_image!="") {
					$(this).css({
						"background-image": "url(" + this.settings.background_image + ")",
						"background-size": "100% 100%"
					});
				} else {
					$(this).css({"background-image" : "url(" + tspImg + ")"});
				}
			}
		...
		this.each(function() {
			...
			if ( action === "start") {
				...
				//brush dialog
				currentCanvas.$brushToolbox = plugin.create_toolbox.call(currentCanvas,"brush",{ left: $(currentCanvas).parent().offset().left, top: $(currentCanvas).parent().offset().top },currentCanvas.settings.brushes_title,80);

				// overwrite brush properties as defined in defaults
				if(!$.isEmptyObject(currentCanvas.settings.buttons)) {
					$.each($.fn.drawr.availableBrushes,function(i,brush){
						if(currentCanvas.settings.buttons.hasOwnProperty(brush.name)) {
							var button=currentCanvas.settings.buttons[brush.name];
							let keys = Object.keys(button);
							for (let j = 0; j < keys.length; j++) {
								brush[keys[j]] = button[keys[j]];
							}
							if(keys.length>0)
								$.fn.drawr.availableBrushes[i]=brush;
						}
					});
				}
				...
				// create buttons
				$.each($.fn.drawr.availableBrushes,function(i,brush){
					if($.isEmptyObject(currentCanvas.settings.buttons) || currentCanvas.settings.buttons.hasOwnProperty(brush.name)) {
				...
				if($.isEmptyObject(currentCanvas.settings.buttons) || currentCanvas.settings.buttons.hasOwnProperty("palette")) {
				...
				currentCanvas.$undoButton=plugin.create_button.call(currentCanvas,currentCanvas.$brushToolbox[0],"action",{"icon":"mdi mdi-undo-variant mdi-24px"}).on("touchstart.drawr mousedown.drawr",function(){
					if(currentCanvas.undoStack.length>0){
						...
						if(currentCanvas.undoStack.length==0) {//don't allow stack to be emtpy.
							currentCanvas.$undoButton.css("color","#999999");
				...
				} else if(currentCanvas.settings.color_mode=="picker"){
