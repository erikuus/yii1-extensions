 ...
 *        'last_name',
 *        array(
 *            'class'=>'ext.widgets.grid.expandcolumn.XExpandColumn',
 *            'name'=>'sort',
 ...
 * }
 *
 * XExpandColumn class file is a clone of original TbRelationalColumn class file from YiiBooster toolkit
 * {@link https://github.com/clevertech/YiiBooster}. There are only minor changes that decouple it from
 * toolkit dependecies.
 *
...
class XExpandColumn extends CDataColumn
{
	/**
	...
	/**
	 * @var string $columnCssClass the class name that will wrap up the cell content.
	 * Important Note: this class will be used as the trigger for the AJAX call, so make sure is unique for the
	 * column.
	 */
	public $columnCssClass = 'expand-column';

	/**
	 * @var string $rowCssClass the class name that will wrap up the new row that spans all columns.
	 */
	public $rowCssClass = 'expand-row';
	...
	public function renderDataCell($row)
	{
		...
		echo CHtml::openTag('span', array('class' => $this->columnCssClass, 'data-rowid' => $this->getPrimaryKey($data)));
